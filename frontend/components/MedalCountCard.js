    ...
    <div className="bg-surface rounded-lg shadow-theme-md p-6">
      ...
      <div className="space-y-3">
        {topCountries.map((country, index) => (
          <div
            key={country.id}
            className="flex items-center justify-between py-2 border-b border-light last:border-b-0"
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="ml-2 font-semibold text-secondary min-w-[2rem] text-right">
                {country.total}
              </span>
            </div>
          </div>
...
function MedalBadge({ emoji, count, label }) {
  return (
    ...
    >
      <span>{emoji}</span>
      <span className="text-body min-w-[1.5rem] text-center">{count}</span>
    </span>
  );
}
